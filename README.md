...
| 백엔드 초점 | OpenAI 호환 API, 정책 pipeline, semantic cache, quota/fallback, request trace |
...
| 성과 | 상태 | 근거 |
|---|---|---|
| OpenAI 호환 진입점 | `/v1/chat/completions`와 batch API 3종 구현 | backend offline tests |
| 정책 pipeline | auth, quota, guardrail, cache, router, dispatch, fallback, record 흐름 구성 | pipeline tests |
| 캐시/라우팅 비교 | exact/semantic cache와 deterministic A/B/fallback mode 제공 | Phase 2 local extension tests |
| 비용 없는 데모 | fake provider와 static web console로 라우팅/trace 재현 | `web/scripts/verify-static-demo.mjs` |
...
| 포인트 | 선택 이유 | 구현/검증 |
|---|---|---|
| OpenAI-compatible facade | 클라이언트 변경 없이 gateway 정책을 끼워 넣기 위함 | `/v1/chat/completions` endpoint tests |
| fake provider 기본값 | 실제 LLM 비용 없이 policy pipeline을 반복 검증하기 위함 | offline backend regression |
| semantic cache | 유사 요청 비용과 지연을 줄이는 gateway 책임을 보여주기 위함 | cache tests, static demo |
| fallback/retry budget | provider 실패를 앱별 구현이 아니라 gateway 기준으로 처리하기 위함 | `ROUTED_RESILIENT` mode tests |
...
|---|---|---|---|
| provider 직접 연동 보류 | live provider를 먼저 붙이면 비용/secret/네트워크가 회귀 검증을 흔듦 | fake provider와 deterministic embedding 기본값 | offline regression, static demo |
| WebFlux gateway | gateway는 외부 API 호환성과 비동기 IO가 중요함 | Spring Boot WebFlux | backend tests |
| Redis cache 선택 검증 | semantic cache는 Redis/Testcontainers 경로가 필요하지만 기본 검증과 분리해야 함 | Redis tests를 opt-in으로 분리 | Colima env 기반 `Redis*` tests |
...
| 항목 | 내용 | 근거 |
|---|---|---|
| 로컬 기본 | fake provider, in-memory/local deterministic path | offline backend regression |
| Redis 선택 경로 | semantic cache Redis 검증 | `Redis*` Testcontainers tests |
| Static demo | dependency-free web console | `npm run build`, `npm test` |
| Infra draft | PostgreSQL+pgvector, Redis, Nginx compose | `docker compose ... config` |
...
| 영역 | 증명하려는 역량 | 결과 |
|---|---|---|
| Gateway API | OpenAI 호환 endpoint와 batch 흐름 설계 | 4개 구현 API |
| 정책 pipeline | auth/quota/cache/router/fallback/trace 구성 | backend regression |
| local-first 검증 | 비용 없는 fake provider와 static demo 유지 | web verification, offline tests |
